1.  Install following the steps:
    1). sudo git clone https://github.com/forevercqd/vundle.git ~/vundle
    2). sudo cp -rf  ~/vundle/.vim*  ~/
    3)  sudo git clone   https://github.com/gmarik/Vundle.vim.git    ~/.vim/bundle/Vundle.vim
...
基本快捷键：
    ...
    5) cscope 查找结果
       ...
       注意按下 CTRL+\+g 就是没作用。其实是先按ctrl不放，按\键，同时放开，快速按g键！
        ...
        cscope 用法：       
        :cs find s smbole_A // 查找 smbole_A 出现的地方
        :cw         // 跳回查找结果
        :cs show    // 查看
        
        另外在 .vimrc 添加 set cscopequickfix=c-,d-,e-,g-,i-,s-,t- 后，可使用下面命令
        :copen  // 打开查找结果;
        :cclose // 关闭查找结果;
        :cnext  // 跳转到下一查找结果;
        :cprev  // 跳转到上一查找结果;
